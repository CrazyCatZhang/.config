...
sketchy_topbar --add item back left \
	...
	blur_radisu=22 \
	padding_left=2 \
	background.border_width=0 click_script="$BACK_SCRIPT" \
	--subscribe back mouse.clicked # background.color=$COLOR_CYAN_DULL_TWO \
...
sketchy_topbar --add item pause left \
	...
	background.drawing=on \
	\
	background.border_width=0 background.height=30 \
	padding_left=-2 \
